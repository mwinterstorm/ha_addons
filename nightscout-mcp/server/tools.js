...
export function createMcp(config, ns) {
  ...
  register('get_treatments', 'Read recorded treatments with source units. No inferred totals; free-text notes and personal identifiers are omitted.', history, args => readHistory('treatments.json', args, 'created_at', treatment));
  register('get_carbs_history', 'Read carbohydrate-containing treatment records (carbs in grams). Filtering follows the source-record limit; shorten truncated windows.', history, args => readHistory('treatments.json', args, 'created_at', treatment, row => typeof row.carbs === 'number'));
